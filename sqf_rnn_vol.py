...
np.savez('sqf_rnn_train_loss', train_loss)
np.savez('sqf_rnn_test_loss', test_loss)
...
#checkpoint = torch.load('sqf_vol_saved.tar')
...
window_freq = int(((test_vol.shape[1]/64)-((T+pred_length)/64))/num_test_windows)*64
  ...
  for  j in range(number_series): 
      for k in range(num_test_windows): 
        ...
        for n in range(num_paths): 
          ...
          paths[j,k,n, T:] = sample_path

#sqf_train_loss = train_loss
#sqf_paths = paths    
np.savez('test_data', norm_test_windows)
np.savez('sqf_rnn_test_loss', test_loss)
np.savez('sqf_rnn_paths', paths)
