...
optimiser = torch.optim.Adam(list(global_model.parameters()) + list(local_model.parameters()), lr = learning_rate)
...
#checkpoint = torch.load('df_vol_saved.tar')
#global_model.load_state_dict(checkpoint['global_state_dict'])
#local_model.load_state_dict(checkpoint['local_state_dict'])
...
norm_test_windows = np.zeros((number_series, num_test_windows, T+pred_length))
window_freq = int((test_vol.shape[1]-(T+pred_length))/num_test_windows)
for j in range(num_test_windows):
    if j == num_test_windows - 1: 
        index = test_vol.shape[1] - (T+pred_length)
    else: 
        index = j*window_freq
    print(index)
    norm_test_windows[:,j,:] = new_norm_test_vols[:,index:index+(T+pred_length)]
...
with torch.no_grad(): 
  for i in range(num_test_windows): 
      ...
      for n in range(num_paths): 
        ...
        fixed_effects_saved.append(fixed_effects)
...
np.savez('DF_fixed_effects', fixed_effects_saved)
np.savez('DF_test_data_adam', norm_test_windows)
np.savez('DF_train_loss_adam', train_loss)
np.savez('DF_test_loss_adam', test_loss)
np.savez('DF_vol_paths_adam', paths)
