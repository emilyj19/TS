...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 28 10:40:37 2019
...
def create_train_ratios_data(data, num_series, len_series, num_windows): 
...
training_ratio = create_train_ratios_data(train_ratios[:,:-int(T/64)], number_series, T, number_windows)
...
np.savez('new_model_train_loss_ratio', train_loss)
np.savez('new_model_test_loss_ratio', test_loss)
...
test_windows = np.zeros((number_series, num_test_windows, T))
window_freq = int((test_ratio.shape[1]-T)/num_test_windows)
for j in range(num_test_windows):
    if j == num_test_windows - 1: 
        index = test_ratio.shape[1] - T
    else: 
        index = j*window_freq
    test_windows[:,j,:] = test_ratio[:,index:index+T]
    ...
    for  j in range(number_series): 
        ...
        for k in range(num_test_windows): 
            ...
            for n in range(num_paths): 
                ...
                unconstrained_paths[j,k,n,:T_encoder] = input_test_data[j][k][:T_encoder,0]
                unconstrained_paths[j,k,n, T_encoder:] = sample_path.numpy()
                constrained_sample_path = torch.nn.functional.softmax(sample_path)
                paths[j,k,n, T_encoder:] = constrained_sample_path.numpy()
...
np.savez('new_model_test_series_ratio', test_windows)
np.savez('attn_weights_ratio_batch', attn_weights)
np.savez('new_model_paths_ratio', paths)
np.savez('new_model_unconstrained_paths_ratio', unconstrained_paths)
